...
/*   Updated: 2025/08/05 16:58:19 by aautret          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief  découpe une seule chaîne d’arguments (contenant plusieurs nombres
 * séparés par des espaces) en plusieurs chaînes individuelles représentant
 * chaque nombre
 *
 * @param str
 * @return char**
 */
static char	**parsing_split_one_arg(char *str)
{
	char	**res;
...
/**
 * @brief  duplique les chaînes d’arguments déjà séparées dans une nouvelle zone
 * mémoire pour les manipuler indépendamment
 *
 * @param ac
 * @param av
 * @return char**
 */
static char	**parsing_copy_args(int ac, char **av)
{
	char	**res;
	int		i;

	res = malloc(sizeof(char *) * (ac - 1 + 1));
	if (!res)
		return (NULL);
	i = 0;
	while (i < ac - 1)
...
char	**parsing_split_args(int ac, char **av)
{
	if (ac == 1)
		return (NULL);
	if (ac == 2)
		return (parsing_split_one_arg(av[1]));
	return (parsing_copy_args(ac, av));
}
